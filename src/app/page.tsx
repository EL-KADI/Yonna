...
export default function Home() {
  ...
  // الحصول على التوكن فقط في الكلينت
  const getToken = () => {
  ...
  const navigateToUserProfile = (
    ...
    router.push(
      `/user/${userId}?name=${encodeURIComponent(
        userName
      )}&photo=${encodeURIComponent(userPhoto || "")}`
    );
  };
  ...
  const formatDate = (dateString: string) => {
    // استخدام تنسيق ثابت أثناء SSR
    if (typeof window === "undefined") {
      return new Date(dateString).toISOString().split("T")[0]; // تنسيق ثابت (YYYY-MM-DD)
    }
  ...
  const fetchPosts = async (pageNumber: number) => {
    try {
      ...
      if (!response.ok) {
        ...
        console.log("Error fetching posts:", errorData);
  ...
  const loadMorePosts = useCallback(async () => {
    if (loadingMore || (paginationInfo && page >= paginationInfo.numberOfPages))
      return;
